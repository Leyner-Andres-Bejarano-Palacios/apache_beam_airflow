...
if __name__ == "__main__":
    ...
    dimTipoSolicitud  = (
        ...
        | 'Query Table Clientes' >> beam.io.ReadFromBigQuery(
            query='''
                select * from(SELECT distinct TIPO_SOLICITUD as TipoSolicitud, GENERATE_UUID() as TipoSolicitudID
                FROM`afiliados-pensionados-prote.afiliados_pensionados.FuturaPensionados`
                group by
                TipoSolicitud)
                where TipoSolicitud
                not in
                (select TipoSolicitud from `afiliados-pensionados-prote.afiliados_pensionados.FuturaPensionados`)
                ''',\
