...
if __name__ == "__main__":
    ...
    dimInfPersonas  = (
        ...
        | 'Query Table Clientes' >> beam.io.ReadFromBigQuery(
            query='''
            SELECT GENERATE_UUID() as InfPersonasID, BeneficiariosId,TipoPersona,PensionadosId, d.SolicitudesId, CalculosActuarialesId,
            concat(BeneficiariosId,TipoPersona,PensionadosId,CalculosActuarialesId)
            ...
            on
            d.Numero_Solicitud= pen.NUMERO_SOLICITUD
            WHERE concat(BeneficiariosId,TipoPersona,PensionadosId,CalculosActuarialesId)
            NOT IN
            (SELECT concat(BeneficiariosId,TipoPersona,PensionadosId,CalculosActuarialesId) FROM `afiliados-pensionados-prote.Datamart.diminfpersonas`)
                ''',\
