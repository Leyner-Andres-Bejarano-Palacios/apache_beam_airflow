...
def fn_check_date_format(element,key,dateFormat):
    if (element[key] is not None  and element[key] != "None"): 
        try:
...
if __name__ == "__main__":
    ...
    dimBeneficiarios  = (
        ...
        | 'Query Table Beneficiarios' >> beam.io.ReadFromBigQuery(
            query='''
                select a.BeneficiariosId,a.BeneficiariosId,a.TipoIdentificacionBeneficiario,
                a.IdentificacionBeneficiario,a.FechaDato,a.IdentificacionAfiliado,a.fechaFinTemporalidad,
                a.ConsecutivoPension,a.SecuenciaBeneficiario,a.CalidadBeneficiario,
                a.SecuenciaBeneficiario,a.CalidadBeneficiario,a.Sexo,a.FechasNacimiento,a.TipoBeneficiario,
                a.SubtipoBeneficiario,a.Parentesco,a.Nombre,a.primerNombre,a.segundoNombre,a.primerApellido,a.segundoApellido, a.dtmfechanacimiento,b.filtrovalidation,
                vg.prueba
                from(SELECT GENERATE_UUID() as BeneficiariosId,TIPO_PENSION AS TipoPension, a.TIPO_ID_BENEFICIARIO as TipoIdentificacionBeneficiario, a.ID_BENEFICIARIO as IdentificacionBeneficiario,
                ...
                count(TIPO_ID_BENEFICIARIO) as aparicionesID, count(Nombre) as aparicionesNombre, case when count(TIPO_ID_BENEFICIARIO) >1 then 'reportar' end as validacion
                ...
                GROUP by TipoPension,TipoIdentificacionBeneficiario, IdentificacionBeneficiario,
                FechaDato,IdentificacionAfiliado,fechaFinTemporalidad,
                ConsecutivoPension, SecuenciaBeneficiario, CalidadBeneficiario,
                Sexo, FechasNacimiento, TipoBeneficiario, SubtipoBeneficiario, Parentesco, Nombre, primerNombre,segundoNombre,primerApellido,segundoApellido, dtmfechanacimiento
                having count(DocumentoDeLaPersona)>0 and count(Nombre)>0)a
                left join
                (select*from(select ID_BENEFICIARIO,case when count(ID_BENEFICIARIO) >1 then ID_BENEFICIARIO end as filtrovalidation from afiliados-pensionados-prote.afiliados_pensionados.PenFutura
                group by ID_BENEFICIARIO
                having count(ID_BENEFICIARIO)>0) where filtrovalidation is not null
                ) as b
                on
                a.IdentificacionBeneficiario=b.ID_BENEFICIARIO

                left join
                (select count(filtrovalidation) as prueba, ID_BENEFICIARIO as identifBen from (select*from(select ID_BENEFICIARIO,case when count(ID_BENEFICIARIO) >1 then ID_BENEFICIARIO end as filtrovalidation from afiliados-pensionados-prote.afiliados_pensionados.PenFutura
                group by ID_BENEFICIARIO
                having count(ID_BENEFICIARIO)>0) where filtrovalidation is not null
                )
                group by identifBen
                having count(filtrovalidation)>1
                ) as vg
                on
                a.IdentificacionBeneficiario=vg.identifBen
                ''',\
    ...
    IdentificacionBeneficiario_qualityRule30_newNumeration = IdentificacionBeneficiario_biggerThan_validated | 'repeated identification with different identification document type' >> beam.Map(fn_repeatedId_different_docType)

    fechaFinTemporalidad_dateFormat_validated = IdentificacionBeneficiario_qualityRule30_newNumeration | 'date format fechaFinTemporalidad' >> beam.Map(fn_check_date_format,    'fechaFinTemporalidad', "%Y%m%d" )
